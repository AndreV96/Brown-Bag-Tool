...
- **`index.html`** — the dashboard. No sidebar; just a header and `<main>` holding three tabs (Upcoming Sessions, Past Presentations, Documentation), one visible at a time via `hidden`. Inline JS filters `window.SESSIONS` at render time into the two session-based tabs, by comparing each session's date against the current date (via `new Date()`, not a hardcoded value):
  ...
  - **Documentation** — static content (not derived from `window.SESSIONS`): what a Brown Bag is, its format, and how to prepare one.
...
- On interaction, `alert()` a short explanation that it's a preview/placeholder (see the Schedule Presentation modal and the resource links on `session.html` for examples).
